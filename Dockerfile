...
ADD https://api.github.com/repos/yashmanian/cartographer/git/refs/heads/master?access_token=$github_token \
...
RUN pip install statistics numpy requests

ARG gl
RUN if  (test -n "$gl"); then sudo apt-get install -y libnvidia-gl-$gl; else echo "Will not install nvidia GL" ; fi
